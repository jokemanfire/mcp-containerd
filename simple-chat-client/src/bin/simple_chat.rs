...
async fn main() -> Result<()> {
    ...
    // create chat session
    let mut session = ChatSession::new(openai_client, tool_set, config.model_name.unwrap_or_else(|| "nebulacoder-v6.0".to_string()));

    // build system prompt with tool info
    let mut system_prompt = "你是一个助手，可以帮助用户完成各种任务。你有以下工具可以使用：\n".to_string();
